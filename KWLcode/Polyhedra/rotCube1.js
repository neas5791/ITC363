...
// Develops on statCube1.js and generates the modelview matrix in the
// application.
// Author Ken Lodge 13/07/2015
...
// An enumeration to identify axes
var xAxis = 0;
var yAxis = 1;
var zAxis = 2;

var axis = 0;			// current spin axis
var theta = [0, 0, 0];	// initial orientation

var mv = mat4();		// The modelview matrix
...
function render() {
	...
	theta[axis] += 2.0;
	// Calculate modelview matrix as Rx * Ry * Rz
	mv = mult(rotate(theta[yAxis], [0,1,0]), rotate(theta[zAxis], [0,0,1]));
	mv = mult(rotate(theta[xAxis], [1,0,0]), mv);
