...
function Block(location, scales) {
    var rs = mult(scalem(scales));
...
Block.prototype.render = function(offset, worldview) {
    ...
    gl.drawArrays(gl.TRIANGLES, offset, Tree.NV);
};
...
Block.initModel = function() {
    ...
    for ( var j = 0; j < Tree.faces ; j++ ){
        upper.push( vec4( Math.cos( j * omega ) / -2.0, Math.sin( j * omega ) / 2.0, 0.1, 1.0 ) );
        lower.push( vec4( Math.cos( j * omega ) / -2.0, Math.sin( j * omega ) / 2.0, 0.0, 1.0 ) );
    }
    ...
    for (var i = 0; i < Tree.faces; i++){
        vertices[i + 1] = vec4( Math.cos( j * omega) / 6.0, Math.sin(j * omega) / 6.0, 0.1, 1.0 ); // scale the rawVertices
    }
...
function doTrees() {
    ...
    var scales = vec3( 1.0, 1.0, Math.random() * 3.0);   // stone faces along x-direction
